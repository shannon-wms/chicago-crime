...
```{r}
iucr_codes = read_csv("data/iucr_codes.csv")
iucr_codes
iucr_codes %>%
  mutate(description = paste(`PRIMARY DESCRIPTION`, `SECONDARY DESCRIPTION`))
```
...
violent = descs[str_detect(descs, "SEX") |
                    ...
                    (str_detect(descs, "AGG") & !str_detect(descs, "NON-AGG"))
                ]
