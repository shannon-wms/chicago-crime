...
We shall use a grid search with 5-fold cross validation to choose the bandwidth hyperparameter for the radial basis function kernel.
...
gam1$fit_summary
pred <- gam$predict(X = bind_rows(X_train, X_test))
...
base_gam_plot <- ggplot(all_dat) + 
  ...
  geom_line(aes(date, pred), size = 2) + 
...
count_train_new[1, "n_pre"] <- data_2014[data_2014$date == "2014-12-31", "n"][1,] #fill in previous year manually
count_test_new[1, "n_pre"] <- count_train_new[count_train_new$date == "2018-12-31", "n"] #fill in previous year manually
...
gam_plot2 <- ggplot(all_dat) + geom_point(aes(date, n)) + 
  xlab("Date") + ylab("Daily crimes") + 
  geom_line(aes(date, predict(gam2, type = "response", newdata = all_dat), 
                colour = Type), size = 2, alpha = 0.4) + 
  ggtitle("Daily GAM") + 
  theme(legend.position = c(0.9, 0.9), 
        legend.background = element_rect(fill = "lightgrey", size = 0.5, 
                                         linetype = "solid"), 
        ...
        legend.text = element_text(size = 10))
# Obtain metrics on test data
gam2_pred <- predict(gam2, type = "response", newdata = count_test_new)
rmse[2] <- rmse_loss(gam2_pred, count_test_new$n)
rsq[2] <- rsquared(gam2_pred, count_test_new$n)
```
...
gam_plot3 <- ggplot(all_dat) + geom_point(aes(date, n)) + 
  xlab("Date") + ylab("Daily crimes") + 
  geom_line(aes(date, predict(gam3, type = "response", newdata = all_dat), 
                colour = Type), size = 2, alpha = 0.4) + 
  ggtitle("Daily GAM") + t
heme(legend.position = c(0.9, 0.9), 
     legend.background = element_rect(fill = "lightgrey", size = 0.5, 
                                      linetype = "solid"), 
     legend.title = element_text(size = 13), 
     legend.text = element_text(size = 10))
# Obtain metrics on test data
gam3_pred <- predict(gam3, type = "response", newdata = count_test_new)
rmse[3] <- rmse_loss(gam3_pred, count_test_new$n)
rsq[3] <- r_squared(gam3_pred, count_test_new$n)
```
...
gam_plot4 <- ggplot(all_dat) + geom_point(aes(date, n)) + 
  xlab("Date") + ylab("Daily crimes") + 
  geom_line(aes(date, predict(gam4, type = "response", newdata = all_dat), 
                colour = Type), size = 2, alpha = 0.4) + 
  ggtitle("Daily GAM") + 
  theme(legend.position = c(0.9, 0.9), 
        legend.background = element_rect(fill = "lightgrey", size = 0.5, 
        ...
        legend.text = element_text(size = 10))
# Obtain metrics on test data
gam4_pred <- predict(gam4, type = "response", newdata = count_test_new)
rmse[4] <- rmse_loss(gam4_pred, count_test_new$n)
rsq[4] <- r_squared(gam4_pred, count_test_new$n)
```
...
best_gam_plot <- ggplot(all_dat) + geom_point(aes(date, n)) + 
  xlab("Date") + ylab("Daily crimes") + 
  geom_line(aes(date, predict(gam5, type = "response", newdata = all_dat), 
                colour = Type), size = 2, alpha = 0.4) + 
  ggtitle("Daily GAM") + 
  theme(legend.position = c(0.9, 0.9), 
        legend.background = element_rect(fill = "lightgrey", size = 0.5, 
        ...
        legend.text = element_text(size = 10))
# Obtain metrics on test data
gam5_pred <- predict(gam5, type = "response", newdata = count_test_new)
rmse[5] <- rmse_loss(gam5_pred, count_test_data$n)
rsq[5] <- r_squared(gam5_pred, count_test_data)

plot(predict(gam5, type = "response", newdata = count_test_new), count_test_new$n)
abline(b = 1, a = 0, col = "red")

gam_metrics <- data.frame(RMSE = rmse, R2 = rsq)
xtable::xtable(gam_metrics)
```

### Adding Spatial and Categorical Data
...
X_test <- spatial_train %>% select(-n)
```

```{r, eval = FALSE}
# Find CV error for given metrics
gam_results <- kfold_cv(wgam_com, X = X, y = y, error_funcs = metrics, 
                        k = 5, n_reps = 5, parallel = TRUE, n_threads = 5)
names(gam_results) <- c("Root MSE", "R2")
```

```{r, include = FALSE}
gam_results <- readRDS("temp_data/GAM_results.rds")
wgam_com <- readRDS("temp_data/wGAM.rds")
```

```{r}
tab <- xtable::xtable(as_tibble(gam_results), digits = 4)
print(tab, type = "latex")
...
```{r}
spatial_test$predicted <- wGAM_com$predict(X = X_test)

rmse_loss(spatial_test$n, spatial_test$predicted)
r_squared(spatial_test$n, spatial_test$predicted)
