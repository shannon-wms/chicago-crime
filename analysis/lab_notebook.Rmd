...
rmse_loss(pred_test, y_test)
r_squared(pred_test, y_test)
```
...
```{r}
gam2 <- gam(n ~ s(as.numeric(yday), bs = "cc") + n_pre, data = count_train_new, family = "poisson")
...
gam2_pred <- rmse_loss(predict(gam2, type = "response"), newdata = count_test_new)
...
```{r}
gam3 <- gam(n ~ s(as.numeric(yday), bs = "cc") + s(as.numeric(dow), bs = "cr", k = 5) + n_pre, data = count_train_new, family = "poisson")
...
```
```{r}
gam4 <- gam(n ~ s(as.numeric(yday), bs = "cc") + as.factor(dow) + n_pre, data = count_train_new, family = "poisson")
...
gam5 <- gam(n ~ s(as.numeric(yday), bs = "cc") + as.factor(dow) + n_pre + is_fom + is_christmas + 
               is_NYD + as.factor(week) , data = count_train_new, family = "poisson")
