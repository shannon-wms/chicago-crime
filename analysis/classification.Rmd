...
X <- df %>% select(-arrest)
y <- df$arrest
idxs <- sample(1:nrow(df), 60000)
X <- X[idxs, ]
y <- y[idxs]


# Fit to larger subset and predict on full 2019 data
...
# Get metrics
svm_results <- 
classification_accuracy(svm$predictions, df$arrest)
classification_sensitivity(as.numeric(svm$predictions) - 1, as.numeric(df$arrest))
classification_specificity(as.numeric(svm$predictions) - 1, as.numeric(df$arrest))
```
...
classification_accuracy(as.numeric(predict(rf, newdata = df))-1, df$arrest)
classification_sensitivity(as.numeric(predict(rf, newdata = df))-1, as.numeric(df$arrest))
classification_specificity(as.numeric(predict(rf, newdata = df))-1, as.numeric(df$arrest))
