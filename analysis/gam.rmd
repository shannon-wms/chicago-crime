...
spatial_train <- gam_train %>% 
  ...
  count(month, year, community_area) %>%
...
GAM_results <- kfold_cv(mGAM, X =X, y = y, error_funcs = metrics, k = 5, n_reps = 5, parallel = TRUE, n_threads = 5)
names(GAM_results) <- c("Root MSE", "R2")
xtable(as_tibble(GAM_results), digits = 3)

# Fit model and find validation error
mGAM$fit(X, y, n_threads = 7)
mGAM$gam_fitted
mGAM$fit_summary


spatial_test$predicted <- predict(mGAM$gam_fitted, type = "response", newdata = spatial_test)
...
val_rmse 
val_r2
xtable(data.frame("Root MSE" = val_rmse, "R2" = val_r2))
```

```{r, eval = T, echo = F}
mGAM <- ("mGAMcv.rds")
GAM_results <- readRDS("mGAM.rds")
...
wGAM_com <- readRDS("wGAM.rds")
xtable(as_tibble(GAM_results), digits = 4)
GAM_results <- readRDS("GAM_results.rds")
val_rmse <- rmse_loss(spatial_test$n, spatial_test$predicted)
val_r2 <- r_squared(spatial_test$n, spatial_test$predicted)
