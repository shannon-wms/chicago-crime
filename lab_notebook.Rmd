...
3. Regression: Predicting the number of reported crimes which occur in a given region over a specified time period (of a given type, if desired), through interpolation. We consider kernel ridge regression and generalised additive models (GAMs).

# Workflow and Computational Techniques

In the development of the package **chigcrim**, we utilised a number of computational techniques which are documented below. The package can be accessed on GitHub: \url{https://github.com/shannon-wms/chicago-crime/tree/main/chigcrim}.
...
The `R6` class `KernelRegression` fits a kernel ridge regression model with a specified kernel: one of `"linear"` (equivalent to ridge regression), `"polynomial"`, or `"rbf"` (radial basis kernel).
...
for (i in 1:length(bandwidth)){
  kr = KernelRidge$new(kernel = "rbf", lambda = 0.001, bandwidth[i])
...
Note that, whilst kernel ridge regression is useful for interpolation, this model with a radial basis kernel actually has limited utility for forecasting reported crime counts into the future. We illustrate this point by attempting to extrapolate from this model.
...
Our `R6` class `PoissonGAM` is a wrapper for the Poisson family of GAMs, fitted via `gam` from the package **mgcv**. For full details on its use, see the package documentation.

For training the GAMs we will be using data from between 2015 and 2018. For testing we shall use data from 2019.
...
We first consider a GAM applied to daily counts of reported crime incidents, aggregated over the entire City of Chicago.
...
We separate this dataset into a training set and a test set, and proceed to fit our baseline GAM which has the year and day of the year as predictors. We use a cyclic cubic smoother for the day of the year, and proceed to predict on both the training and test data.
...
# Initialise vectors for RMSE and R squared values to compare subsequent models
rmse <- c()
rsq <- c()
...
There are several features that may be of interest. For example: whether a day falls on a weekend; the number of crime incidents reported on the previous day; or the number of crimes on the same day of the previous year. 
...
At first glance, we notice that the highest number of reported crimes often falls on the first day of the year. We also find that the lowest number appears to fall on Christmas day. We constructed the functions `add_prev_day`, `add_dow`, `add_is_fom`, `add_is_christmas` and `add_is_nyd` to attempt to capture some of this information. We proceed to fit a few different GAMs on the resulting data and observe their explanatory and predictive power.
...
As `PoissonGAM` does not currently have functionality to support additional custom features, we fit the following GAMs directly using `gam` from **mgcv**. The same analysis is performed on the following models.
...
The final model appears to have the best performance, with the lowest RMSE and highest $R^2$ value. This model takes as input the most features, so is likely able to capture the variance to a greater extent.

\pagebreak

### Adding Spatial Data

In order to compare our model with the kernel ridge regression model, we aggregate over months and add a spatial component: the community area. `PoissonGAM` has functionality for taking a neighbourhood list along with the community area data, and fits a Markov random field smoother to this factor via `gam`.
...
Notice that this model explains `98.1%` of the variance in the training data.
...
To illustrate the predictive power of the fitted model over the community areas, we sum the true number of crimes reported in the test dataset and the predicted number generated by the model. Below we produce a map of the residuals.
...
In most areas, the model performs very well with relatively low errors. However, there are specific areas in which the predicted counts are far higher or lower than the actual counts.

```{r, include = FALSE}
ggsave("by_area1.pdf", by_area)
```

### Adding Categorical Data

On further exploratory analysis we found that adding the neighbourhood list did not significantly improve upon the model which took community area as a factor with no neighbourhood list provided. In this final section we fit a GAM without the neighbourhood list, with an additional factor indicating the type of crime reported.
...
The updated model has a lower RMSE value but also a lower $R^2$ value. Below we produce a map in the same way as the previous section.
...
The maps are quite similar, which may indicate little improvement in the model from adding the categorical component.